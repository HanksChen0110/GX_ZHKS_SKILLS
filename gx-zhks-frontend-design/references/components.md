...
- Desktop layout: fixed dark sidebar on the left, light workspace on the right.
- Sidebar logo: rounded cyan outline icon tile plus two-line brand text.
- Sidebar sections: primary nav, 最近使用, 最近对话, footer status.
...
- Inner panel: lighter translucent surface with cyan border and faint glow.
- Hero title: navy-to-cyan gradient text, centered.
- Subtitle: 15-16px slate text.
- Prompt bar: white card, 16px radius, light cyan outer ring, dark primary action button.
- Mobile: title wraps to two lines; prompt action can become a compact vertical button if space is tight.
...
- The input composer is a white prompt bar with an embedded dark primary send button.
